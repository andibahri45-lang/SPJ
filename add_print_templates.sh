...
# We'll use sed to insert kwitansi_besar and bast_rekapan before the final '</div> ))} </>'
# I will output the new templates to a file and insert it.

cat << 'INNER_EOF' > new_templates.txt
      ...
      {type === 'bast_rekapan' && rekapBelanja && rekapItems && rekapItems.length > 0 && (() => {
        ...
        <div id="print-bast-rekapan" className="space-y-6 font-serif text-[12px] leading-relaxed">
          ...
          <p className="text-justify indent-8">
            Pada hari ini {getNamaHari(firstP.tgl_penetapan, firstP.bulan_penetapan, firstP.tahun_penetapan)}, tanggal {terbilang(parseInt(firstP.tgl_penetapan, 10) || 0)} bulan {titleCase(firstP.bulan_penetapan)} tahun {terbilang(parseInt(firstP.tahun_penetapan, 10) || 0)} kami yang bertanda tangan di bawah ini :
          </p>
          ...
          <p className="text-justify indent-8">
            <span className="font-bold">PIHAK KESATU</span> menyerahkan kepada <span className="font-bold">PIHAK KEDUA</span> barang sesuai dengan Nota Pesanan No {firstP.no_nota} tanggal {firstP.tgl_penetapan} {firstP.bulan_penetapan} {firstP.tahun_penetapan} dengan jenis barang sebagai berikut:
          </p>
...
# Insert into file
sed -i -e '/<\/div>      ))}    <\/>/r new_templates.txt' src/components/PrintTemplates.tsx
