...
bastRekapanContent = bastRekapanContent.replace(/setNoBast\(\`900\\\/BASTRekapan\\\/\$\{currentRekap\.id_rekap\}\\/BKPPD-HS\\\/\$\{tahunPenetapan\}\`\)/g, "setNoBast(`900/BAST/${currentRekap.id_rekap}/BKPPD-HS/${tahunPenetapan}`)");
...
bastRekapanContent = bastRekapanContent.replace(/setNamaUsaha\(currentRekap\.kepada\);/, `
        ...
        const relatedPesanan = pesananList?.find(p => currentRekap.pesanan_ids?.includes(p.id_pesanan));
...
kwitansiContent = kwitansiContent.replace(/kwitansi/g, 'kwitansiBesar'); // Note: this might lowercase KwitansiBesar incorrectly, but Kwitansi -> KwitansiBesar handles most. kwitansi -> kwitansiBesar
