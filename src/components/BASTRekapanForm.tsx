...
import { Pesanan, RekapBelanja, RekapBelanja, BASTRekapan, INDONESIAN_MONTHS, Rekanan, PejabatKeuangan } from '../types';
import { Save, X, RefreshCw, Printer, AlertTriangle, FileCheck, Calendar } from 'lucide-react';
import DatePicker from './DatePicker';

interface BASTRekapanFormProps {
  pesananList?: Pesanan[];
  pesananList?: Pesanan[];
  isOpen: boolean;
  onClose: () => void;
  currentRekap: RekapBelanja | undefined;
  bastRekapanList: BASTRekapan[];
  rekananList: Rekanan[];
  pejabatKeuanganList: PejabatKeuangan[];
  onSaveBASTRekapan: (bast: BASTRekapan) => void;
  onTriggerPrint?: (docType: 'nota' | 'balasan' | 'kwitansi' | 'bast' | 'sewa') => void;
}

export default function BASTRekapanForm({
  isOpen,
  onClose,
  currentRekap,
  bastRekapanList,
  ...
  onSaveBASTRekapan,
  onTriggerPrint
}: BASTRekapanFormProps) {
  // Core BASTRekapan fields
  ...
  useEffect(() => {
    if (currentRekap && isOpen) {
      const existing = bastRekapanList.find(b => b.id_rekap === currentRekap.id_rekap);
      ...
      if (existing) {
        ...
        setNoBast(`900/BAST/${currentRekap.id_rekap}/BKPPD-HS/${tahunPenetapan}`);
        ...
        setNamaPemilik(currentRekap.pemilik_toko || relatedPesanan?.nama_pemilik || '');
        setAlamatPemilik(relatedPesanan?.alamat_pemilik || '');
        setNamaMengetahui(relatedPesanan?.kepala_kantor || '');
        setNipMengetahui(relatedPesanan?.nip_kepala_kantor || '');

        // Auto generate handover clauses (Replicates Cari_Keterangan_Click)
        const clauses = `PIHAK KESATU menyerahkan belanja barang kepada PIHAK KEDUA, dan PIHAK KEDUA menyatakan telah menerima dalam keadaan baik, lengkap, dan baru berupa ${currentRekap.uraian_belanja_gabungan}.`;
        setKeterangan(clauses);
      }
    }
  }, [currentRekap, bastRekapanList, isOpen]);
  ...
  const handleSaveAndPrint = (shouldPrint: boolean) => {
    ...
    const payload: BASTRekapan = {
      id_bast: `bast_${currentRekap.id_rekap}`,
    ...
    onSaveBASTRekapan(payload);
    
    if (shouldPrint && onTriggerPrint) {
      setTimeout(() => {
        onTriggerPrint('bast_rekapan');
      }, 150);
    } else {
      alert(`Data BASTRekapan Serah Terima untuk No. Urut ${currentRekap.id_rekap} Berhasil Disimpan!`);
    }
    onClose();
  };
    ...
    <div className="fixed inset-0 z-50 overflow-y-auto flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm">
      <div className="relative w-full max-w-4xl bg-[#111827] border border-slate-800 rounded-3xl shadow-2xl overflow-hidden text-slate-100">
        ...
        <div className="px-6 py-4 border-b border-slate-800 flex items-center justify-between bg-slate-900/60">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-sm font-bold text-white uppercase tracking-wider font-mono">
              Berita Acara Serah Terima (BASTRekapan)
            </h3>
          </div>
          <button 
            onClick={onClose}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
            ...
            <div className="lg:col-span-8 space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                <div>
                  <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">No. BASTRekapan (Surat)</label>
                  <input
                    type="text"
                    value={noBast}
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
          ...
          <div className="md:col-span-2">
            <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">Tanggal BASTRekapan</label>
            <DatePicker
              value={getDateString()}
      ...
      <div className="lg:col-span-4 space-y-6">
        ...
        <div className="bg-[#0F172A] border border-indigo-950 p-4 rounded-xl text-indigo-300 space-y-2">
          ...
          <p className="text-[11px] leading-relaxed text-slate-400">
            BASTRekapan ini bertindak sebagai berkas verifikasi fisik bahwa kuantitas barang yang tertera di nota pesanan telah diterima sepenuhnya oleh pengurus aset BKPPD.
          </p>
        </div>
      </div>
        </div>
