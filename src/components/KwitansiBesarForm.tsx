...
import { Pesanan, RekapBelanja, RekapBelanja, KwitansiBesar, terbilang, formatNumberWithSeparator, parseSeparatorToNumber } from '../types';
...
interface KwitansiBesarFormProps {
  pesananList?: Pesanan[];
  isOpen?: boolean;
  onClose?: () => void;
  currentRekap: RekapBelanja | undefined;
  kwitansiBesarList: KwitansiBesar[];
  onSaveKwitansiBesar: (kwitansiBesar: KwitansiBesar) => void;
  onTriggerPrint?: (type: 'nota' | 'balasan' | 'kwitansiBesar' | 'bast' | 'sewa') => void;
}

export default function KwitansiBesarForm({
  isOpen,
  onClose,
  currentRekap,
  kwitansiBesarList,
  ...
  useEffect(() => {
    if (currentRekap) {
      // Look if there's already a saved kwitansiBesar for this PO
      const existing = kwitansiBesarList.find(k => k.id_rekap === currentRekap.id_rekap);
      ...
      if (existing) {
        ...
        setNoBukti(`KW/${currentRekap.id_rekap}/BKPPD-HS/${tahun}`);
        setUangSejumlah(currentRekap.total_bruto);
        setYangMenerima(currentRekap.nama_pemilik || currentRekap.kepada);
        setAlamatPenerima(relatedPesanan?.alamat_pemilik || '');
        
        // Auto-generate Untuk Pembayaran text as default
        setUntukPembayaran(`Pembayaran Belanja ${currentRekap.uraian_belanja} Guna Kebutuhan Kantor BKPPD Kabupaten Halmahera Selatan.`);
      }
    }
  }, [currentRekap, kwitansiBesarList]);

  // Append button handlers matching the VBA TambahKata_Click hooks
  const appendBelanja = () => {
    if (currentRekap) {
      setUntukPembayaran(prev => `${prev} ${currentRekap.uraian_belanja}.`);
    }
  };
  ...
  const appendSubKeg = () => {
    if (currentRekap) {
      setUntukPembayaran(prev => `${prev} Pada Sub Kegiatan: ${currentRekap.sub_kegiatan}.`);
    }
  };

  const handleSaveAndPrint = (shouldPrint: boolean) => {
    if (!currentRekap) {
      alert('Pilih/Simpan data Pesanan terlebih dahulu sebelum menyimpan kwitansiBesar.');
      return;
    }

    const payload: KwitansiBesar = {
      id_kwitansiBesar: `kw_${currentRekap.id_rekap}`,
      id_rekap: currentRekap.id_rekap,
      no_bukti: noBukti,
      npwp,
      kode_org: kodeOrg,
      ...
      bendahara: currentRekap.bendahara,
      nip_bendahara: currentRekap.nip_bendahara,
      kepala_kantor: currentRekap.kepala_kantor,
      nip_kepala_kantor: currentRekap.nip_kepala_kantor,
      pengurus,
    ...
    onSaveKwitansiBesar(payload);
    
    if (shouldPrint && onTriggerPrint) {
      setTimeout(() => {
        onTriggerPrint('kwitansi_besar');
      }, 150);
    } else {
      alert(`Data KwitansiBesar/Tanda Bukti untuk No. Urut ${currentRekap.id_rekap} Berhasil Disimpan!`);
    }
  };
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm">
        <div className="bg-[#1E293B] border border-amber-500/30 text-amber-200 rounded-xl p-5 flex items-center gap-3 shadow-lg relative max-w-lg w-full">
          ...
          <div>
            ...
            <p className="text-[11px] text-slate-400 mt-0.5">KwitansiBesar bergantung pada relasi data Pesanan. Silakan isi dan simpan Data Pesanan pada tab pertama terlebih dahulu.</p>
          </div>
        </div>
      </div>
    );
    ...
    <div className="fixed inset-0 z-50 overflow-y-auto flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm">
      <div className="relative w-full max-w-5xl bg-[#111827] border border-slate-800 rounded-3xl shadow-2xl overflow-hidden text-slate-100">
        ...
        <div className="px-6 py-4 border-b border-slate-800 flex items-center justify-between bg-slate-900/60">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-sm font-bold text-white uppercase tracking-wider font-mono">
              Input KwitansiBesar / Tanda Bukti Kas
            </h3>
          </div>
          <button 
            onClick={onClose}
        ...
        <div className="p-6 space-y-6 max-h-[80vh] overflow-y-auto">
          ...
          Formulir Isian KwitansiBesar / Tanda Bukti
        </h3>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          <div>
            <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">No. Bukti KwitansiBesar</label>
            <input
              type="text"
              value={noBukti}
      ...
      <div className="lg:col-span-5 space-y-6">
        ...
        <div className="bg-[#0F172A] border border-indigo-950 p-4 rounded-xl text-indigo-300 space-y-2.5">
          ...
          <p className="text-[11px] leading-relaxed text-slate-400">
            KwitansiBesar ini dibuat otomatis berdasarkan data transaksi dari <strong className="text-indigo-300 font-mono">Nota No. Urut {currentRekap.id_rekap}</strong>.
          </p>
          <ul className="text-[10px] font-mono space-y-1 bg-indigo-950/40 p-2.5 rounded-lg border border-indigo-900/30 text-indigo-300">
            <li className="truncate">Sub Keg: {currentRekap.sub_kegiatan}</li>
            <li className="truncate">Belanja: {currentRekap.uraian_belanja}</li>
            <li>Total Bruto: {formatRupiah(currentRekap.total_bruto)}</li>
            <li>Netto Bayar: {formatRupiah(currentRekap.jumlah_bersih)}</li>
          </ul>
        </div>
      </div>
    </div>
