...
import { DaftarHarga, formatNumberWithSeparator, parseSeparatorToNumber } from '../types';
...
interface DaftarHargaFormProps {
  daftarHargaList: DaftarHarga[];
  onSave: (items: DaftarHarga[]) => void;
}

export default function DaftarHargaForm({ daftarHargaList, onSave }: DaftarHargaFormProps) {
  ...
  const [pendingImportItems, setPendingImportItems] = useState<DaftarHarga[] | null>(null);
  
  // Modal states
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [editingItem, setEditingItem] = useState<DaftarHarga | null>(null);
  ...
  const handleOpenEdit = (item: DaftarHarga) => {
    setEditingItem(item);
  ...
  const handleSaveItem = (e: React.FormEvent) => {
    ...
    if (!namaBarang || !satuan) {
      alert('Nama Barang dan Satuan wajib diisi.');
      return;
    }

    const newItem: DaftarHarga = {
      id: editingItem ? editingItem.id : 'h_' + Date.now() + '_' + Math.random().toString(36).substr(2, 5),
      nama_barang: namaBarang.trim(),
      satuan: satuan.trim(),
      harga_satuan: Number(hargaSatuan) || 0
    };

    let updatedList: DaftarHarga[];
    if (editingItem) {
      updatedList = daftarHargaList.map(item => 
        item.id === editingItem.id ? newItem : item
      );
    } else {
      updatedList = [...daftarHargaList, newItem];
    }
  ...
  const executeDelete = (id: string) => {
    const updated = daftarHargaList.filter(item => item.id !== id);
    onSave(updated);
  ...
  const executeBulkDelete = () => {
    const updated = daftarHargaList.filter(item => !selectedIds.includes(item.id));
    onSave(updated);
    setSelectedIds([]);
  ...
  const filteredList = daftarHargaList.filter(item => {
  ...
  const handleExportCSV = () => {
    if (daftarHargaList.length === 0) {
      alert('Tidak ada data untuk diexport.');
      return;
    }
    const headers = ['Nama Barang', 'Satuan', 'Harga Satuan'];
    const rows = daftarHargaList.map(item => [
    ...
    link.setAttribute("download", `data_master_daftar_harga_${new Date().getFullYear()}.csv`);
  ...
  const handleDownloadTemplate = () => {
    const headers = ['Nama Barang', 'Satuan', 'Harga Satuan'];
    const sampleRow = [
      '"Amplop (Executiv) Bergaris Pakai tali"',
      '"buah"',
      '"2000"'
    ];
    ...
    const link = document.createElement("a");
    link.setAttribute("href", url);
    link.setAttribute("download", "template_daftar_harga.csv");
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    URL.revokeObjectURL(url);
  ...
  const handleUploadCSV = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      try {
        ...
        const newItems: DaftarHarga[] = [];
        for (let i = 1; i < lines.length; i++) {
          const cols = parseCSVLine(lines[i], delimiter);

          if (cols.length >= 2) {
            // Clean pricing string (remove dots, format, etc.)
            let cleanPriceStr = cols[2] ? cols[2].replace(/[^\d]/g, '') : '0';
            let parsedPrice = Number(cleanPriceStr) || 0;

            newItems.push({
              id: 'h_' + i + '_' + Date.now() + '_' + Math.random().toString(36).substr(2, 3),
              nama_barang: cols[0],
              satuan: cols[1] || 'Buah',
              harga_satuan: parsedPrice
            });
          }
        }
  ...
  const handleConfirmImport = (mode: 'merge' | 'overwrite') => {
    ...
    if (mode === 'overwrite') {
      ...
      alert(`Berhasil mengunggah ${pendingImportItems.length} data daftar harga (Menggantikan seluruh data lama).`);
    } else {
      // Merge: match based on item name (lowercase) to prevent exact duplicates
      const existingMap = new Map(daftarHargaList.map(item => [item.nama_barang.toLowerCase().trim(), item]));
      pendingImportItems.forEach(item => {
        existingMap.set(item.nama_barang.toLowerCase().trim(), item);
      });
      onSave(Array.from(existingMap.values()));
      alert(`Berhasil menggabungkan ${pendingImportItems.length} data daftar harga baru.`);
    }
  ...
  return (
    <div className="space-y-6">
      {/* Title & Tambah Daftar Harga Button */}
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-slate-900 border border-slate-800 rounded-xl p-5 text-white shadow-md">
        <div>
          <h1 className="text-xl font-extrabold text-white tracking-tight flex items-center gap-2">
            ...
            Data Master Daftar Harga
          </h1>
          <p className="text-xs text-slate-400 mt-1">
            Manajemen daftar harga satuan barang, material, jasa, dan satuan operasional.
          </p>
        </div>
        <button
      ...
      <div className="bg-[#1E293B] border border-slate-700/80 rounded-xl shadow-lg overflow-hidden">
          ...
          <table className="w-full text-left border-collapse text-xs text-slate-300">
            <thead className="bg-slate-900/80 text-slate-400 font-semibold border-b border-slate-800 uppercase tracking-wider text-[10px]">
              <tr>
                ...
                <th className="px-4 py-3.5 min-w-[240px]">Nama Barang</th>
                <th className="px-4 py-3.5 w-44">Satuan</th>
                <th className="px-4 py-3.5 w-44">Harga Satuan</th>
              ...
              {paginatedList.length === 0 ? (
                <tr>
                  <td colSpan={5} className="text-center py-10 text-slate-500 italic">
                    {searchQuery ? 'Tidak ada barang yang cocok dengan kata kunci pencarian.' : 'Belum ada data daftar harga master.'}
                  </td>
                </tr>
              ) : (
        ...
        <div className="p-3 border-t border-slate-800 bg-slate-900/60 flex flex-col sm:flex-row justify-between items-center gap-2 text-[10px] text-slate-400 font-mono">
          <div>
            Menampilkan {filteredList.length === 0 ? 0 : (currentPage - 1) * pageSize + 1} sampai {Math.min(currentPage * pageSize, filteredList.length)} dari {filteredList.length} data disaring (Total master: {daftarHargaList.length})
          </div>
          <div>
            Rata-rata Harga Barang: {formatRupiah(daftarHargaList.length === 0 ? 0 : Math.round(daftarHargaList.reduce((sum, item) => sum + item.harga_satuan, 0) / daftarHargaList.length))}
          </div>
        </div>
      </div>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-fade-in">
          <div className="bg-[#1E293B] border border-slate-700 rounded-xl shadow-2xl max-w-lg w-full overflow-hidden">
            {/* Modal Header */}
            <div className="flex items-center justify-between p-4 border-b border-slate-700 bg-slate-900">
              <h3 className="font-extrabold text-white text-sm uppercase tracking-wide flex items-center gap-1.5">
                ...
                {editingItem ? 'Edit Daftar Harga' : 'Tambah Daftar Harga'}
              </h3>
              <button
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-1.5">
                  ...
                  <input
                    ...
                    placeholder="Contoh: Amplop (Executiv) Bergaris Pakai tali"
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <input
                      ...
                      placeholder="Contoh: buah, Dos, Rim, Botol"
                      className="w-full px-3 py-2 bg-[#0F172A] border border-slate-700 rounded-lg text-slate-200 placeholder-slate-500 focus:outline-none focus:border-indigo-500"
                      required
                    />
                  </div>
                  ...
                  <div className="space-y-1.5">
                    ...
                    <input
                      type="text"
                      value={formatNumberWithSeparator(hargaSatuan)}
                      onChange={(e) => setHargaSatuan(parseSeparatorToNumber(e.target.value))}
            ...
            <div className="p-5 text-center space-y-4">
              ...
              <div className="space-y-1">
                ...
                <p className="text-xs text-slate-400">
                  Apakah Anda yakin ingin menghapus data master barang ini? Tindakan ini tidak dapat dibatalkan.
                </p>
              </div>
          ...
          <div className="bg-[#1E293B] border border-slate-700 rounded-xl shadow-2xl max-w-sm w-full overflow-hidden">
            <div className="p-5 text-center space-y-4">
              ...
              <div className="space-y-1">
                ...
                <p className="text-xs text-slate-400">
                  Apakah Anda yakin ingin menghapus <strong className="text-white">{selectedIds.length}</strong> data barang yang dicentang? Tindakan ini tidak dapat dibatalkan.
                </p>
              </div>
              <div className="flex items-center gap-2 pt-2">
                <button
            ...
            <div className="p-6 text-center space-y-4">
              ...
              <div className="space-y-2">
                ...
                <p className="text-xs text-slate-300">
                  Ditemukan <span className="text-indigo-400 font-bold font-mono">{pendingImportItems.length}</span> data barang dari file. Bagaimana Anda ingin memproses data ini?
                </p>
                <div className="text-[11px] text-slate-400 text-left bg-slate-900/60 p-3 rounded-lg space-y-1.5 border border-slate-800">
                  <p>&bull; <strong className="text-emerald-400">Gabungkan (Rekomendasi):</strong> Menambahkan data baru dan memperbarui data lama jika nama barang sama. Data lama lainnya tetap utuh.</p>
