...
sed -i -e '/{docTypes.map((type, index) => {/a \
        if (type === "kwitansi_besar") {\
          if (!rekapBelanja) return null;\
          return (\
            <div key={`${type}-${index}`} className="print-page w-full min-h-screen bg-white p-8 font-sans text-black relative break-after-page">\
              <div className="absolute right-8 top-8 font-serif font-bold text-sm">\
                Lembar : 1\
              </div>\
        ...
        \
        if (!pesanan || !items) return null;' src/components/PrintTemplates.tsx

bash update_print_templates.sh
